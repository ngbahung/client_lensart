...
const CustomerListPage = () => {
  ...
  const handleSearch = (value) => {
    ...
    const filtered = customers.filter(customer => 
      customer.id.toString().toLowerCase().includes(value.toLowerCase()) ||
    ...
    <div className="bg-white p-6 rounded-md flex flex-col min-h-[calc(100vh-120px)]">
      <div className="flex-grow">
        <div className="grid grid-cols-1 gap-4">
          <Table 
            customers={getCurrentPageItems()}
            isLoading={isLoading}
            error={error}
            onStatusChange={handleStatusChange}
            onSearch={handleSearch}
            searchTerm={searchTerm}
            onUpdate={fetchCustomers} // Truyền trực tiếp function fetchCustomers
          />
        </div>
      </div>
