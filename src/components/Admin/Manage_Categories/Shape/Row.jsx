...
const Row = ({ shape, onStatusChange, onEdit }) => {
  ...
  const handleConfirmStatusChange = async () => {
    setIsUpdating(true);
    try {
      await onStatusChange(shape.id, !shape.status);
  ...
  return (
    <>
      <tr className="hover:bg-gray-100 h-[48px]">
        <td className="py-2 px-4">{shape.id}</td>
        <td className="py-2 px-4">{shape.name}</td>
        <td className="py-2 px-4">
          <ToggleSwitch
            id={shape.id}
            status={shape.status} // Truyền trực tiếp status string không cần convert
