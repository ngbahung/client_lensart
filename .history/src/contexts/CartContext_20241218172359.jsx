...
const transformCartData = (cartDetails) => {
  return cartDetails.map(item => ({
    ...
    // Calculate total_price locally instead of using API value
      ...
      const updatedItems = state.items.map(item =>
        item.id === action.payload.id 
          ? { 
              ...
              total_price: parseFloat(action.payload.price) * action.payload.quantity
            } 
          : item
      );
      return {
...
export const CartProvider = ({ children }) => {
  ...
  const updateCartItem = async (cartDetailId, product_id, branch_id, color, quantity) => {
    dispatch({ type: 'UPDATE_CART_ITEM_START', payload: cartDetailId });
    try {
      ...
      if (result.success) {
        ...
        dispatch({ type: 'UPDATE_CART_ITEM_ERROR', payload: cartDetailId });
