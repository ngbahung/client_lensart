  ...
  switch (action.type) {
    ...
    case 'SELECT_CART_ITEM': {
      ...
      const itemsWithUpdatedSelection = state.items.map(item =>
        item.id === id ? { ...item, selected: value !== undefined ? value : !item.selected } : item
      );
      return {
        ...state,
...
export const CartProvider = ({ children }) => {
  ...
  const selectCartItem = (itemId, value) => {
    dispatch({ type: 'SELECT_CART_ITEM', payload: { id: itemId, value } });
  };
