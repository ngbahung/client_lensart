                ...
                <div className="container mx-auto px-4 ">
                    <div className="flex items-center justify-between h-16">
                            ...
                            <nav className="hidden md:flex space-x-4 lg:space-x-8">
                                {navItems.map((item) => (
                                    <div key={item.path} className="relative group">
                                        <Link
                                            to={item.path}
                                            className="flex items-center text-gray-600 hover:text-[#F5A97F] transition-colors text-sm lg:text-base pb-2"
                                        >
                                            {item.name}
                                        ...
                                        <div className="absolute left-0 invisible group-hover:visible opacity-0 group-hover:opacity-100 
                                            transition-all duration-300 ease-in-out mt-0 w-48 bg-white border rounded-md shadow-lg z-50">
                                            <div className="py-2">
                                                {item.subItems.map((subItem, index) => (
                                                    <Link
                                                        key={index}
                                                        to={`${item.path}/${subItem.toLowerCase().replace(/\s+/g, '-')}`}
                                                        className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-[#F5A97F]"
                                                    >
                                                        {subItem}
                                                    </Link>
                                                ))}
                        ...
                        <div className="hidden md:flex items-center justify-end space-x-4 lg:space-x-8 py-3">
                            <Link 
                                to="/ve-lensart" 
                                className="text-gray-700 hover:text-[#F5A97F] transition-colors text-sm lg:text-base"
                            >
                                Về LensArt
                            </Link>
                            ...
                            <div className="relative group">
                                <Link 
                                    to="/blog" 
                                    className="flex items-center text-gray-700 hover:text-[#F5A97F] transition-colors text-sm lg:text-base"
                                >
                                    Blog
                                    <BiChevronDown className="h-5 w-5 ml-1 transition-transform group-hover:rotate-180" />
                                </Link>
