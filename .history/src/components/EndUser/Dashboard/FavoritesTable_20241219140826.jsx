...
function FavoritesTable() {
  ...
  const handleRemoveFavorite = async (wishlistDetailId, productName) => {
    ...
    try {
      setDeletingId(wishlistDetailId); // Set loading state for this item
  ...
  const MobileProductCard = ({ product }) => (
    <div className="bg-white p-4 rounded-lg shadow mb-4 border">
      <div className="flex items-center space-x-4">
        ...
        <button
          onClick={() => handleRemoveFavorite(product.wishlist_detail_id, product.product_name)}
          className="text-red-600 p-2 hover:bg-red-50 rounded-full transition-colors disabled:opacity-50"
          disabled={deletingId === product.wishlist_detail_id}
        >
          {deletingId === product.wishlist_detail_id ? 'Đang xóa...' : 'Xóa'}
        </button>
      </div>
    </div>
  );
            ...
            {favorites.map((product) => (
              <tr key={product.wishlist_detail_id} className="border-b hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm">
                  <button
                    onClick={() => handleRemoveFavorite(product.wishlist_detail_id, product.product_name)}
                    className="text-red-600 hover:text-red-900 font-medium hover:bg-red-50 px-3 py-1 rounded-md transition-colors disabled:opacity-50"
                    disabled={deletingId === product.wishlist_detail_id}
                  >
                    {deletingId === product.wishlist_detail_id ? 'Đang xóa...' : 'Xóa'}
                  </button>
