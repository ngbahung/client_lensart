...
const ShoppingCart = ({ onContinueShopping }) => {
  ...
  return (
    <div className="p-4 md:p-8 space-y-4 md:space-y-8"></div>
      {Object.entries(groupedItems).map(([branchId, branch]) => (
        <div key={branchId} className="border-2 border-[#55d5d2] rounded-lg overflow-hidden"></div>
                ...
                onSelect={() => {
                  ...
                  if (!item.selected) {
                    cartItems.forEach(cartItem => {
                      if (cartItem.branch_id !== branchId) {
