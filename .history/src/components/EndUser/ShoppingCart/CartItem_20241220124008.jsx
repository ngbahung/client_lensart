...
const CartItem = ({ item, onSelect }) => {
  const { updateCartItem, removeCartItem, updatingItems, selectedBranchId } = useCart();
  const isUpdating = updatingItems[item.id];
  const isDisabled = selectedBranchId && selectedBranchId !== item.branch_id;
  ...
  return (
    <div className={`flex flex-col sm:flex-row items-start sm:items-center justify-between py-3 md:py-4 px-3 md:px-4 border-2 
      ${isDisabled ? 'border-gray-200 bg-gray-50 opacity-60' : 'border-[#55d5d2] bg-[#eff9f9]'} 
      rounded-lg mb-2`}>
