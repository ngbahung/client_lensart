...
const GongKinhPage = ({ categoryId = 1, pageTitle = "Gọng Kính" }) => {
    ...
    const [filters, setFilters] = useState({
        features: [], // Change style to features
        material: [],
        gender: [],
        priceRange: [],
        brands: [],
    ...
    const filterProducts = (products) => {
        return products.filter(product => {
            ...
            const matchesFeatures = filters.features.length === 0 || 
              filters.features.includes(product.style); // Change style to features
            ...
            return matchesFeatures && 
                   ...
                   matchesPriceRange && 
                   matchesBrand &&
                   matchesShape; // Add shape filter condition
          });
    };
    ...
    useEffect(() => {
        const fetchFilterData = async () => {
            try {
                const [brands, features, materials, shapes] = await Promise.all([
                    getBrands(),
                    getFeatures(),
                    getMaterials(),
                ...
                setFilterOptions(prev => ({
                    ...
                    features: features.map(feature => feature.name), // Change style to features
                    material: materials.map(material => material.name),
                    shapes: shapes.map(shape => shape.name) // Add shapes to filter options
                }));
            } catch (error) {
                ...
                <div className={`w-full lg:w-1/4 ${isFilterOpen ? '' : 'hidden lg:block'}`}>
                    <SideBar 
                        ...
                        filterOptions={{
                            ...
                            shapes: {
                                title: "Hình dạng",
                                options: filterOptions.shapes // Add shapes to filter options
                            }
                        }}
                    />
                </div>
