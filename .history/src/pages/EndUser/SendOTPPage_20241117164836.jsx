...
const VerificationInput = ({ value, onChange, error }) => (
  <div className="mb-4">
    <input
      type="text"
      value={value}
      ...
      className={`w-full px-4 py-3 rounded-full bg-[#F8FBFF] border ${
        error ? 'border-red-400' : 'border-[#E8F0FE]'
      } focus:outline-none focus:border-teal-400 placeholder:text-gray-400 text-center`}
    />
    {error && (
...
const VerificationButton = ({ onClick, loading }) => (
  <button
    onClick={onClick}
    disabled={loading}
    className={`w-full bg-teal-400 text-white py-3 rounded-full transition-all
      ${loading ? 'opacity-70 cursor-not-allowed' : 'hover:bg-teal-500'}
    `}
  >
