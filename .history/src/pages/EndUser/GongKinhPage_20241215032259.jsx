...
const GongKinhPage = ({ categoryId = 2, pageTitle = "Gọng Kính" }) => {
    ...
    useEffect(() => {
        if (type && value) {
            ...
            switch (type) {
                case 'material':
                    newTitle = `${pageTitle} ${decodeURIComponent(value)}`;
                    break;
                case 'style':
                    newTitle = `${pageTitle} ${decodeURIComponent(value)}`;
                    break;
            ...
            document.title = `${newTitle} | LensArt`;
        } else {
            setTitle(pageTitle);
            document.title = `${pageTitle} | LensArt`;
        }
    }, [type, value, pageTitle]);
    ...
    return (
        <div className="mt-1 container mx-auto px-2 sm:px-4">
            <Breadcrumb items={["Trang chủ", title]} />
            <PromotionalBanner 
