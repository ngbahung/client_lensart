...
### 🔧 Cập nhật Workflow File (Nếu Azure đã tự động tạo)

Nếu Azure đã tự động tạo file `.github/workflows/azure-static-web-apps-*.yml`, bạn cần cập nhật để inject environment variables:

#### Bước 1: Mở workflow file
...
Tìm phần `jobs:` và thêm section `env:` như sau:
...
git commit -m "chore: update workflow with env variables"
git push origin main
```

**Kiểm tra**: Vào GitHub Actions để xem workflow chạy với environment variables mới.
...
### 1. Deployment token ở đâu? Tại sao không thấy trong Configuration?

**Trả lời**: 
- Deployment token **KHÔNG** ở trong Settings > Configuration
- Nó ở trong **Overview** > "Manage deployment token" button
- Hoặc trong **Settings** > **Properties**
- Nếu dùng GitHub integration, token **tự động** được thêm vào GitHub Secrets, không cần copy thủ công

### 2. Tôi đã thêm environment variables nhưng vẫn không hoạt động?

**Kiểm tra**:
- [ ] Tên variable có bắt đầu với `VITE_` không? (VD: `VITE_API_BASE_URL`)
- [ ] Đã thêm vào workflow file chưa? (section `env:`)
- [ ] Đã rebuild project chưa? (`npm run build`)
- [ ] Đã clear cache chưa? (`rm -rf node_modules/.vite`)

### 3. GitHub Actions workflow fails với "Invalid token"?

**Giải pháp**:
1. Kiểm tra tên secret trong workflow file khớp với GitHub Secrets
2. Reset token trong Azure Portal và update GitHub Secret
3. Đảm bảo có đúng quyền trong Azure (Contributor role)

### 4. Nên dùng GitHub Secrets hay Azure Configuration?

**Khuyến nghị**: Dùng **GitHub Secrets**
- ✅ Dễ quản lý và version control
- ✅ Tự động inject vào build
- ✅ Khác nhau cho từng branch/environment

Chỉ dùng Azure Configuration khi cần thay đổi mà không muốn redeploy.

### 5. Làm sao để test local trước khi deploy?
...
# 3. Mở browser: http://localhost:4173
```

### 6. Deploy mất bao lâu?

- **Build time**: 1-3 phút
- **Deploy time**: 30 giây - 2 phút
- **Tổng**: Khoảng 2-5 phút

### 7. Có thể rollback về version trước không?

**Có!** Có 2 cách:

**Cách 1**: Qua Azure Portal
1. Vào Static Web App > Environments
2. Chọn environment cần rollback
3. Xem deployment history và chọn version cũ
...
# Hoặc checkout code cũ và force push (cẩn thận!)
git checkout <old-commit-hash>
git push origin main
```

### 8. Preview environment là gì? Khi nào dùng?

**Preview environment** là môi trường tạm thời cho Pull Request:
- 🔗 URL riêng: `https://your-app-<pr-number>.azurestaticapps.net`
- ✅ Test changes trước khi merge
- ✅ Review với team
- 🗑️ Tự động xóa sau khi close PR

**Khi nào dùng**:
- Phát triển feature mới
- Fix bugs
- Cần review trước khi merge vào main

### 9. Làm sao biết deployment thành công?

**Kiểm tra trên GitHub**:
1. Vào tab **Actions**
2. Workflow có dấu ✅ màu xanh = Thành công
3. Có ❌ màu đỏ = Thất bại (xem logs)

**Kiểm tra trên Azure**:
1. Vào Static Web App > Environments
2. Click vào Production
3. Xem deployment history

**Kiểm tra trực tiếp**:
- Mở URL: `https://your-app.azurestaticapps.net`
- Nếu site load được = Thành công ✅

### 10. Chi phí deploy là bao nhiêu?

**Azure Static Web Apps** có 2 tiers:

| Tier | Giá | Giới hạn |
|------|-----|----------|
| **Free** | $0/tháng | • 100GB bandwidth<br>• 0.5GB storage<br>• 2 custom domains |
| **Standard** | ~$9/tháng | • Unlimited bandwidth<br>• Unlimited storage<br>• Unlimited domains<br>• Authentication |

**Cho dự án nhỏ/học tập**: Free tier là đủ ✅
...
### Community:
- [Azure Static Web Apps Discussions](https://github.com/Azure/static-web-apps/discussions)
- [Stack Overflow - Azure Static Web Apps](https://stackoverflow.com/questions/tagged/azure-static-web-apps)

---

## 📝 Tóm tắt các bước (Checklist)

### Lần đầu setup:

- [ ] **Bước 1**: Tạo Azure Static Web App với GitHub integration
- [ ] **Bước 2**: Xác nhận deployment token đã tự động thêm vào GitHub Secrets
- [ ] **Bước 3**: Thêm environment variables vào GitHub Secrets
- [ ] **Bước 4**: Cập nhật workflow file để inject env variables
- [ ] **Bước 5**: Push code lên main branch
- [ ] **Bước 6**: Kiểm tra GitHub Actions và Azure Portal
- [ ] **Bước 7**: Mở URL để verify deployment
...
> 💡 **Pro tip**: Bookmark file này và file `DEPLOYMENT_WORKFLOW.md` để tham khảo nhanh khi cần!
